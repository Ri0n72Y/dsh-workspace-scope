...
- Skill exclusion uses `ctx.skills.register()` to shadow a farther-layer winning Skill. DSH runtime Skill registrations are first-wins within the same exact layer, so a same-name runtime Skill already registered directly in that Agent layer cannot be replaced by this plugin's shadow. Current DSH first-party production paths do not register such Agent-local runtime Skills; do not add a second catalog/execute guard for this theoretical edge unless the runtime contract changes.
