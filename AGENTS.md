...
Keep the two public entries as composition roots rather than feature containers.

- `src/index.ts`: Host assembly only. Feature logic lives under `src/host/`:
  - `config.ts`: parse/read/write `.dsh-scope.json`
  - `api.ts`: overview/save and HTTP transport
  - `policy.ts`: pure whitelist/blacklist set algebra
  - `mcp.ts`: Host-global MCP inventory/grouping
...
- Two halves. The Host composition root is `src/index.ts`; runtime logic is under `src/host/`. The first turn-owned `system-prompt/assemble` (identified by its explicit Agent plus turn signal) reads and locks `.dsh-scope.json`. Agent-scoped diagnostic assemblies without a turn signal do not lock the blank session. The Host reconciles the effective Host-global MCP deny set before the model request; when the mask changes it gets an Agent-bound ToolRuntime face with `agent.ctx.get("tools")`, installs `restrict({ deny })`, discards that pre-policy assembly, and asks DSH to assemble once more so native and PTC presentation are generated under the restriction. A prepended `agent/pre-step` refreshes excluded Skill shadows before DSH `tool-skill` reads the catalog. The Host also owns overview/save endpoints.
- Cordis service property reads and registry reads are different seams. Do not use raw `agent.ctx.skills` / `agent.ctx.tools` merely because the service exists elsewhere in the Host tree: undeclared property reads resolve only through the current/ancestor fiber stores and can throw `cannot get property ... without inject`. For Agent-scoped mutations use `agent.ctx.get("skills")` / `agent.ctx.get("tools")`; Cordis returns a traceable face rebound to the Agent Context, so service methods still observe the exact Agent scope without creating another inject fiber or cache.
...
- Skill exclusion preserves the winning definition but shadows only model invocation: `modelInvocable:false`, original `userInvocable`. The shadow is registered through the Agent-bound SkillRegistry face returned by `agent.ctx.get("skills")`. DSH `tool-skill` then removes it from both the model catalog and `skill` Tool path while still honoring `/skill-name` when the original Skill is user-invocable.
...
- `dynamic.tsx` is a generated, git-ignored hot-test artifact. `gen-dynamic.mjs` strips imports/exports and stitches the same split Client source modules used by the static build, compiles the same `styles.module.css` class map/stylesheet, then uses TypeScript to syntax-check the result. Its textual stitcher deliberately assumes one-line imports and unique top-level names; replace it with a real bundler only if that ceiling stops holding.
...
- Styles live in `src/client/styles.module.css`, use only `--dsw-*` theme tokens, and are consumed through the imported `css` class map (`css.panel`, `css.card`, etc.); do not write raw styling class strings in TSX.
- The package currently has one controlled CSS Module. `tsdown.config.ts` and `gen-dynamic.mjs` map local camelCase class names to stable `wsc-*` output classes and inject the same stylesheet without an extra CSS dependency. If a second sheet or richer CSS Module semantics such as composition/global escape become necessary, switch to DSH's `@tsdown/css` pipeline instead of extending the local transform.
