...
export function apply(ctx: Context): void {
  ...
  if (webServer === undefined) {
    throw new Error('workspace-scope: webServer is unavailable')
  }
  ...
  async function writeConfig(cwd: string | undefined, cfg: ScopeConfig, session: SessionLike | undefined): Promise<{ saved: boolean; reason?: string }> {
    const fs = ctx.get('fs') as FsServiceLike | undefined
    ...
    try {
      ...
      all.default = cfg
      const sp = ctx.get('sandboxPolicy') as SandboxPolicyServiceLike | undefined
      ...
      const policy = sp !== undefined && session !== undefined
        ? sp.resolve({ session, mode: 'workspace-write' })
        : undefined
      await fs.writeText(target, JSON.stringify(all, null, 2), undefined, undefined, policy)
      return { saved: true }
    } catch (err) {
      return { saved: false, reason: String((err && (err as Error).message) || err) }
    }
  }
  ...
  function renderCatalogText(form: 'catalog' | 'update', entries: Array<{ name: string; description: string }>): string {
    const lines = entries.map((e) => `- \`${e.name}\`: ${escapeHtml(e.description)}`)
    if (form === 'update') {
  ...
  /** Replace the tool-skill catalog message with the workspace-scoped view. */
  function filterCatalogMessages(
    ...
    for (const message of messages) {
      const src = message.source as { kind?: unknown; update?: unknown; entries?: unknown } | undefined
      if (src !== undefined && src.kind === 'skill-catalog' && Array.isArray(src.entries)) {
        const entries = (src.entries as Array<{ name?: unknown; description?: unknown }>)
          .filter((e): e is { name: string; description: string } =>
            typeof e === 'object' && e !== null && typeof e.name === 'string' && typeof e.description === 'string')
  ...
  const appliedRestrictions = new Map<string, { dispose: () => void; deny: string[] }>()
  ...
  async function applyRestriction(agent: AgentLike, cfg: ScopeConfig): Promise<void> {
    ...
    const same = prev !== undefined && prev.deny.length === deny.length
      && deny.every((name, index) => name === prev.deny[index])
    if (same) return
    if (prev !== undefined) {
      try { prev.dispose() } catch { /* already gone */ }
      appliedRestrictions.delete(agent.id)
    }
    if (deny.length === 0) return
    try {
      const tools = (agent as unknown as { ctx: Record<string, unknown> }).ctx.tools as {
        restrict(filter: { deny: string[] }): () => void
      }
      appliedRestrictions.set(agent.id, { dispose: tools.restrict({ deny }), deny })
    } catch (err) {
      console.warn('[workspace-scope] restrict failed:', String((err && (err as Error).message) || err))
    }
  }
  ...
  onEvent('tools/pre-execute', (
    exec: { name?: unknown; agent?: AgentLike; arguments?: Record<string, unknown> },
    next: () => unknown,
  ): unknown => {
    ...
    const excluded = cfgCache.mode === 'whitelist' ? !cfgCache.skills.includes(name) : cfgCache.skills.includes(name)
    if (name !== '' && excluded) {
      return { kind: 'deny', reason: `skill "${name}" is excluded by the workspace scope config` }
    }
    return next()
  })
  ...
  async function saveResult(body: {
    sessionId?: unknown; mode?: unknown; skills?: unknown; mcps?: unknown
  }): Promise<Record<string, unknown>> {
    const sessionId = typeof body.sessionId === 'string' ? body.sessionId : ''
    const agent = resolveAgent(sessionId)
    const cwd = agent?.session.header.cwd
    const cfg: ScopeConfig = {
      mode: body.mode === 'whitelist' || body.mode === 'blacklist' ? body.mode : 'default',
      skills: Array.isArray(body.skills) ? body.skills.filter((x): x is string => typeof x === 'string') : [],
      mcps: Array.isArray(body.mcps) ? body.mcps.filter((x): x is string => typeof x === 'string') : [],
    }
  ...
  const handler = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      const known = path === `${ROUTE_PREFIX}/overview` || path === `${ROUTE_PREFIX}/save`
      if (req.method === 'GET' && path === `${ROUTE_PREFIX}/overview`) {
        send(200, await overviewResult(url.searchParams.get('sessionId') ?? ''))
  ...
  if (typeof harness !== 'undefined') {
    ...
    harness.handle('save', async (args: unknown) => {
      try {
        return await saveResult((args ?? {}) as {
          sessionId?: unknown; mode?: unknown; skills?: unknown; mcps?: unknown
        })
      } catch (err) {
        return { error: String((err && (err as Error).message) || err) }
