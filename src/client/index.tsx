...
function ScopeModal(props: DockProps): React.ReactElement | null {
  ...
  const [collapsed, setCollapsed] = React.useState<{
    skills: boolean;
    mcps: boolean;
  }>({ skills: false, mcps: false });

  const sessionId =
    props.useSessions !== undefined
      ? (props.useSessions((s: any) => s.current as string | undefined) as
          | string
          | undefined)
      : undefined;
    ...
    (next: ScopeDraft): void => {
      ...
      callHost("save", {
        ...
        .catch((err: unknown) => {
          ...
          setNotice({
            kind: "err",
            text: `保存失败：${String((err && (err as Error).message) || err)}`,
          });
        });
    },
    [sessionId],
  ...
  const visibleSkills =
    normalizedQuery === ""
      ...
      : skills.filter(
          (s) =>
            s.name.toLocaleLowerCase().includes(normalizedQuery) ||
            s.description.toLocaleLowerCase().includes(normalizedQuery),
        );
  const visibleMcps =
    normalizedQuery === ""
      ? mcps
      : mcps.filter((m) =>
          m.server.toLocaleLowerCase().includes(normalizedQuery),
        );
      ...
      <li className="wsc-card" data-open={open ? "true" : undefined} key={id}>
        <div className="wsc-card-main">
          ...
          >
            ...
            <span className="wsc-tag" data-enabled={enabled ? "true" : "false"}>
              {enabled ? "已启用" : "已禁用"}
            </span>
            <ChevronIcon className="wsc-chevron" />
          </button>
    ...
    data === null ? (
      ...
      <div className="wsc-body">
        ...
        <div className="wsc-actions">
          ...
          <button type="button" className="wsc-btn" onClick={allDisabled}>
            全部禁用
          </button>
        </div>
        {notice !== null ? (
          <p className={notice.kind === "ok" ? "wsc-notice" : "wsc-error"}>
            {notice.text}
          </p>
        ) : null}
        ...
        <div className="wsc-panel-head">
          ...
          >
            <svg width={14} height={14} viewBox="0 0 14 14" fill="none">
              <path
                fill="currentColor"
                d="M7 5.84863L4.57617 3.4248L3.4248 4.57617L5.84863 7L3.4248 9.42383L4.57617 10.5752L7 8.15137L9.42383 10.5752L10.5752 9.42383L8.15137 7L10.5752 4.57617L9.42383 3.4248L7 5.84863Z"
              />
            </svg>
          </button>
        </div>
